    ...
    def analyze_disease(self, query: str, image_path=None):
        ...
        if isinstance(response, str):
            try:
                ...
                response = CropDiseaseOutput(**data)
            except Exception:
                pass
                
        # Validate the response has the required fields
        is_valid = (
            hasattr(response, "diseases") and 
            hasattr(response, "disease_probabilities") and 
            hasattr(response, "symptoms") and 
            hasattr(response, "Treatments") and 
            hasattr(response, "prevention_tips")
        )
        
        if not is_valid:
            print(f"Warning: Response is not a valid CropDiseaseOutput object. Result was: {response}")
            # Create a default response
            response = CropDiseaseOutput(
                diseases=["Unable to determine"],
                disease_probabilities=[0.0],
                symptoms=["Image analysis failed or invalid response from AI model. Possible quota limit reached."],
                Treatments=["Consult local agricultural expert", "Please try again later"],
                prevention_tips=["Regular crop monitoring recommended"]
            )
        else:
            # Check if all critical fields are empty, still return a fallback
